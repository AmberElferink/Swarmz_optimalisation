...
//constexpr float acosTab[360];
//
//void GenerateLookupTables()
//{
//	//is faster using 256 instead of 360 due to 256 being a power of two
//	for ( int i = 0; i < 360; i++ )
//	{
//		acosTab[i] = (int)( std::acos( i / 180 * PI ) );
//	}
//}
...
struct Vec3
{
	...
	float AngleToNorm( const Vec3 &v)
	{
		return static_cast<float>( std::acos( DotProduct( v ) ) * toRadian ); // number is: 180 / PI
	}
	...
	void checkVoxelForBoids( const Boid &b, std::vector<NearbyBoid> &result, const Vec3 &voxelPos ) const
	{
		...
		if ( iter != voxelCache.end() )
		{
			for ( Boid *test : iter->second )
			{
				...
				float blindAngle = bNegVelocityNorm.AngleToNorm ( distanceVecNorm );

				if ( ( &b ) != test && distance <= PerceptionRadius && ( BlindspotAngleDeg <= blindAngle || bNegVelocityLength == 0 ) )
				{
					NearbyBoid nb;
					nb.boid = test;
